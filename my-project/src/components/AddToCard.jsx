...
{/* <Link id={id} to={`/productDetail/${id}`}> */}
  <div
    ...
    className={`  bg-slate-200 animate-pulse main group bg-red- drop-shadow-lg shadow-black  w-[150px] h-[  sm:w-[270px] sm:h-[370px] my-4 rounded mobile:m-auto transition-all duration-300 `}
  >
    <div
      className={` img sm:w-[270px]  sm:h-[250px]  w-[150px] h-[150px] rounded-sm bg-slate-200 animate-pulse flex flex-col relative center `}
    >
      <button
   ...
   rounded- bg-slate-400 animate-pulse           h-[40px] text-white text-xs sm:text-lg w-[100%] flex   items-end justify-center p-2 rounded-b`}
    ...
    <div className={` myp  sm:w-auto sm:h-auto w-[150px]  h-auto pl-1 gap-0 `}>
      <h2
        className={`
rounded-sm bg-slate-300 animate-pulse
         w-auto h-[30px] sm:text-sm font-medium transition-all duration-300   overflow-hidden text-[10px] bg-yellow   `}
      >
        
      </h2>
      <p className="flex gap-4 my-2 rounded-sm bg-slate-300 animate-pulse w-[100px] h-6 sm:text-base text-[12px]  font-medium text-myTheme">
